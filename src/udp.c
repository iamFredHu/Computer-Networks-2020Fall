...
{
    // TODO
    udp_hdr_t *udp_checksum_hdr = (udp_hdr_t *)buf->data;

    // 1. 你首先调用buf_add_header()添加UDP伪头部
    buf_add_header(buf, sizeof(udp_peso_hdr_t));

    // 2. 将IP头部拷贝出来，暂存被UDP伪头部覆盖的IP头部
    udp_peso_hdr_t *udp_checksum_peso_hdr = (udp_peso_hdr_t *)buf->data;
    udp_peso_hdr_t temp_peso_hdr = *udp_checksum_peso_hdr;

    // 3. 填写UDP伪头部的12字节字段
    memcpy(udp_checksum_peso_hdr->src_ip, src_ip, NET_IP_LEN);
    memcpy(udp_checksum_peso_hdr->dest_ip, dest_ip, NET_IP_LEN);
    udp_checksum_peso_hdr->protocol = NET_PROTOCOL_UDP;
    udp_checksum_peso_hdr->placeholder = 0;
    udp_checksum_peso_hdr->total_len = udp_checksum_hdr->total_len;

    // 4. 计算UDP校验和，注意：UDP校验和覆盖了UDP头部、UDP数据和UDP伪头部
    uint16_t checksum = checksum16((uint16_t *)buf->data, swap16(udp_checksum_hdr->total_len) + sizeof(udp_peso_hdr_t));

    // 5. 再将暂存的IP头部拷贝回来
    *udp_checksum_peso_hdr = temp_peso_hdr;
